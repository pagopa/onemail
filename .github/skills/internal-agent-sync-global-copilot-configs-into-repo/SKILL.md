...
The paired agent should not restate default mode handling, preserved consumer-asset behavior, `internal-sync-*` exclusions, plan-file lifecycle, or automation entrypoints from this skill.
...
- Preserve target `local-*` assets under mirrored categories, preserve any approved repo-specific preserved-asset exceptions, preserve the target consumer-local GitHub instructions overrides file after materialization, and delete target-only assets outside those preserved consumer-owned exceptions during `apply`.
...
- preserved target-owned assets covered by the sync contract, including `local-*` assets, approved repo-specific preserved-asset exceptions, and the consumer-local override file materialized from `.github/copilot-instructions.override.md.template`
